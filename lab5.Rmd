...
```{r}
x <- c("cat", "rat", "bat", "hat")
x[str_detect(x, pattern = "[c|r|b]at")]
```
...
x[str_detect(x, pattern = "[Sh|D]awn|Jon")]
```
...
x[str_detect(x, pattern = "hel{2}o$")]
```
...
str_match(x, pattern = "aa{1,}b{0,}c{1,}")
```
...
x <- c("1 name available?", "2 names available?", "no names found!",
       "22 names available?")
str_match(x, pattern = "^\\d{1,}\\snames?\\savailable\\?")
```
...
str_match(x, pattern = "^My\\sexam\\swas\\ssuccessful$")
```
...
str_match(x, pattern = "\\(?\\d{3}\\)?.*\\d{3}.*\\d{4}")
```
